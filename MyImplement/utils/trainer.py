...
class Trainer:
    def __init__(self, model, loss_func, optimizer, metric, train_loader, valid_loader, test_loader, config):
        ...
        self.to(self.config['TRAIN']['device'])
        ...
        if mode == 'train':
            ...
            with torch.no_grad():
                ...
                if self.config['MODEL']['task'] == 'classification':
                    ...
                    return loss.item(), pred
                elif self.config['MODEL']['task'] == 'recommend':
    ...
    def _compute_metric(self, metric_str, **param_for_rec):
        if self.config['MODEL']['task'] == 'classification':
            ...
            gt_rec_lst = param_for_rec['gt_rec_lst']
            pred_rec_lst = param_for_rec['pred_rec_lst']
            self.metric.compute_metric(gt_rec_lst, pred_rec_lst, eval(self.config['MODEL']['item_num']), eval(self.config['MODEL']['user_num']))
