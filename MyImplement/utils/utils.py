...
class Trainer:
    ...
    def train(self):
        ...
        for e in range(1, epoch + 1):
            all_loss = 0.0
            for s, (batch_y, batch_x) in enumerate(tqdm(self.train_loader)):
            ...
            if e % 1 == 0:
                all_loss = 0.0
                self.metric.init_metric()
                for s, (batch_y, batch_x) in enumerate(tqdm(self.valid_loader)):
    ...
    def test(self):
        all_loss = 0.0
        self.metric.init_metric()
        for s, (batch_y, batch_x) in enumerate(tqdm(self.test_loader)):
            loss, pred = self.step(batch_y.unsqueeze(1), batch_x, mode='evaluate')
            all_loss += loss
